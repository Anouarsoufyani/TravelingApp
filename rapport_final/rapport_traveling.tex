...
pour les données locales, et \textbf{Firebase} (Auth + Firestore) pour la synchronisation
cloud des fonctionnalités sociales. Tous les services utilisés sont gratuits et open-source
(Nominatim, OSRM, Overpass, Open-Meteo, ML Kit, Firebase Spark). Aucune clé API payante
n'est utilisée.

\vspace{6pt}
\begin{tabular}{ll}
  \textbf{Mots-clés :} &
  Android, Java, MVVM, Room, Firebase Auth, Firestore, OSMDroid, \\
  & OSRM, Nominatim, Overpass, Open-Meteo, ML Kit, MediaRecorder, LiveData
\end{tabular}
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item Sync Firestore : création, découverte, demandes, acceptation multi-appareils
...
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item Notifications in-app + badge cloche + sync Firestore (likes, adhésions)
  \item Préférences configurables (AUTEUR / LIEU / TAG / GROUPE) synchronisées Firestore
\end{itemize}
...
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item Plans likés sauvegardés dans Firestore (collection \ic{plans})
\end{itemize}
...
(Model-View-ViewModel)}, en s'appuyant sur les \textbf{composants d'architecture
Android Jetpack} — un ensemble de bibliothèques conçues pour aider les
développeurs à suivre les meilleures pratiques et à séparer les préoccupations.

Les deux ViewModels principaux étendent \ic{AndroidViewModel} (qui reçoit le
contexte \ic{Application} dans son constructeur) plutôt que \ic{ViewModel} simple,
car ils ont besoin d'accéder à \ic{AppDatabase}. Ils survivent aux changements
de configuration (rotation d'écran) : les données ne sont pas perdues et l'UI
n'a pas à les recharger.

Les \ic{LiveData} retournés par les DAOs Room sont des observables \textbf{sensibles
au cycle de vie} (\ic{LifecycleOwner}) : ils ne notifient les Fragments que
lorsque ceux-ci sont à l'état \ic{STARTED} ou \ic{RESUMED}, évitant toute mise à
jour sur une vue détruite. L'abonnement se fait via \ic{observe(getViewLifecycleOwner(), ...)}
directement dans \ic{onCreateView()}.

Pour les cas où plusieurs sources LiveData doivent être fusionnées — par exemple
les plans TravelPath (tous / sauvegardés) — on utilise \ic{MediatorLiveData} :
chaque source est ajoutée via \ic{addSource()}, et seule la source active
propage ses valeurs vers l'UI.
...
\section{La base de données : Room v20}

\textbf{Room} est la bibliothèque de persistance Jetpack — une couche ORM
(Object-Relational Mapping) au-dessus de SQLite qui élimine les requêtes brutes
et garantit la vérification des requêtes à la compilation. Elle repose sur
trois composants annotés :

\begin{itemize}
  \item \ic{@Entity} — définit le schéma d'une table. Chaque classe annotée
    correspond à une table, chaque champ à une colonne.
  \item \ic{@Dao} — interface contenant les méthodes d'accès à la base
    (\ic{@Insert}, \ic{@Query}, \ic{@Delete}). Room génère l'implémentation.
  \item \ic{@Database} — point d'entrée unique, classe abstraite étendant
    \ic{RoomDatabase}, déclarant la liste des entités et la version du schéma.
\end{itemize}

Notre base s'appelle \ic{tp3\_database} (version 20, \ic{fallbackToDestructiveMigration})
et contient \textbf{12 entités}. Les requêtes qui retournent un \ic{LiveData}
sont dites \emph{observables} : Room régénère automatiquement le résultat et
notifie les observateurs à chaque modification de la table concernée, sans
...
appelle \ic{photoDao.searchPhotos("paris")} qui exécute \ic{SELECT ... LIKE '\%paris\%'}
et retourne un \ic{LiveData<List<Photo>>}. Room notifie automatiquement l'UI,
qui met à jour la liste sans qu'il faille écrire de code de rafraîchissement.
\end{tcolorbox}

Toutes les opérations d'écriture s'exécutent sur un pool de \textbf{4 threads
background} (\ic{databaseWriteExecutor = Executors.newFixedThreadPool(4)}),
jamais sur le thread principal — conformément aux contraintes Room.
...
Explorateur, Carte, Publier, Groupes, Profil. Chaque onglet charge un \ic{Fragment}
dans un conteneur central via une \ic{FragmentTransaction} (\ic{replace()} +
\ic{commit()}). Les sous-écrans (détail d'un parcours, chat de groupe…)
s'empilent dans la pile "retour en arrière" via \ic{addToBackStack()}, ce qui
permet à l'utilisateur de revenir à l'écran précédent avec le bouton Retour.
\ic{popBackStack()} est utilisé pour les boutons "←" dans les fragments fils.
...
\begin{tabularx}{\textwidth}{>{\bfseries}l X}
  ...
  Firebase Auth + Firestore
    & Authentification email/password et synchronisation cloud complète de toutes les
      données sociales via \ic{addSnapshotListener} pour les mises à jour temps réel.
      10 collections Firestore : \ic{photos}, \ic{comments}, \ic{users}, \ic{groups},
      \ic{group\_members}, \ic{group\_messages}, \ic{notifications},
      \ic{notification\_prefs}, \ic{plans}. Plan Spark gratuit — aucune carte bancaire. \\
  FirebaseRepository
    & Pattern Repository (recommandé par le cours) : toutes les opérations Firestore
      sont centralisées dans \ic{FirebaseRepository} (singleton), découplant
      complètement Firebase de l'UI. Les Fragments et Activities n'accèdent jamais
      directement au SDK Firestore. \\
  Room (SQLite local)
    & Architecture \textbf{local-first} : Room sert de cache local pour toutes les
      données, garantissant le fonctionnement hors-ligne. Firebase synchronise
      en arrière-plan et Room se met à jour automatiquement via LiveData.
      Les données strictement privées (mots de passe, signalements, étapes de plans)
      restent exclusivement locales. \\
  ...
  Android Jetpack
    & Suite de bibliothèques d'architecture (ViewModel, LiveData, Room) utilisée pour
      séparer les préoccupations, survivre aux changements de configuration et synchroniser
      automatiquement l'UI avec les données via le pattern observateur. \\
  SwipeRefreshLayout
...
Comme les utilisateurs saisissent un pseudo (pas un email), on génère un email Firebase
fictif via \ic{SessionManager.toFirebaseEmail(login)} — par exemple \ic{alice@traveling.app}.
Une fois Firebase Auth validé, l'\ic{userId} Room est récupéré pour alimenter la session locale.
...
L'inscription crée d'abord le compte Firebase avec \ic{createUserWithEmailAndPassword},
puis insère l'utilisateur dans Room. Depuis \ic{MainActivity}, si l'Intent contient
...
L'intégration Firebase couvre l'intégralité des fonctionnalités sociales de l'application.
Toutes les opérations sont centralisées dans \ic{FirebaseRepository} (singleton, pattern Repository).
Les Fragments et Activities n'accèdent jamais directement au SDK Firestore.

\begin{tabularx}{\textwidth}{>{\bfseries}l X}
  \toprule
  Collection Firestore & Stratégie de synchronisation \\
  \midrule
  \ic{photos}
    & Publication → écriture Firestore (doc ID = ID Room).
      Au démarrage de l'explorateur, \ic{syncPublicPhotosToRoom()} tire toutes les photos
      PUBLIC et insère les manquantes dans Room (\ic{insertPhotoOrIgnore}).
      Suppression et modification de titre propagées dans Firestore. \\
  Likes (champ \ic{photos})
    & \ic{updateLikes()} utilise \ic{set(..., merge)} (fonctionne même si le doc n'existe pas).
      \ic{addSnapshotListener} sur le document : compteur temps réel dans \ic{PhotoDetailActivity}.
      Clé \ic{liked\_ids\_\{username\}} dans SharedPreferences — isolation par compte. \\
  \ic{comments}
    & Nouveau commentaire → écrit dans Firestore.
      Suppression → \ic{deleteComment()} interroge Firestore par \ic{photoId+author+text} et supprime.
      \ic{addSnapshotListener} : nouveaux commentaires synchronisés dans Room en temps réel. \\
  \ic{users}
    & Bio sauvegardée à chaque clic "OK" dans \ic{ProfileFragment}.
      Chargée depuis Firestore à l'ouverture si le champ Room est vide. \\
  \ic{groups}
    & Création d'un groupe → doc Firestore \ic{groups/\{name\}}.
      Onglet "Découvrir" : \ic{syncGroupsToRoom()} insère les groupes manquants dans Room. \\
  \ic{group\_members}
    & Demande d'adhésion → doc \ic{\{groupName\}\_\{username\}} avec statut PENDING.
      \ic{listenToPendingMembers()} : listener temps réel chez le créateur, insère les demandes dans Room.
      Accepter → statut MEMBER dans Firestore.
      Onglet "Mes groupes" : \ic{syncMyMemberStatuses()} met à jour Room si accepté sur un autre appareil. \\
  \ic{group\_messages}
    & Chaque message envoyé écrit dans Firestore.
      \ic{listenToMessages()} : listener temps réel dans \ic{GroupChatFragment},
      insère les messages des autres utilisateurs dans Room → LiveData met à jour l'UI. \\
  \ic{notifications}
    & Likes et acceptations d'adhésion → \ic{saveNotification(targetUsername, notif)}.
      \ic{listenToNotifications()} dans \ic{NotificationsFragment} : reçoit les notifications
      des autres appareils et les insère dans Room. \\
  \ic{notification\_prefs}
    & Ajout/suppression d'une préférence → écrit/supprimé dans Firestore.
      Sync au démarrage de \ic{NotificationPreferencesFragment}. \\
  \ic{plans}
    & Plans TravelPath likés → \ic{savePlan(username, plan)} dans Firestore (backup cloud). \\
  \bottomrule
\end{tabularx}
...
\textbf{Limites connues} : les images restent locales (Firebase Storage nécessite une CB
même sur Spark). Sur un autre appareil, les fiches photo s'affichent sans miniature
mais avec toutes les métadonnées texte.

\section{L'explorateur de photos (ExplorerFragment)}

L'explorateur est le fragment principal chargé au démarrage. Il déclenche
immédiatement \ic{viewModel.syncPhotosFromFirestore()} — une synchronisation
silencieuse qui tire les photos PUBLIC de Firestore et insère les manquantes dans Room.
La LiveData Room se met à jour automatiquement, sans indicateur visible.
L'explorateur affiche ensuite un \ic{RecyclerView} en grille à 2 colonnes via
\ic{GridLayoutManager}, alimenté par un \ic{PhotoAdapter} qui charge les images avec Glide. Il propose huit
...
\begin{tabularx}{\textwidth}{>{\bfseries}l X}
  ...
  ❤️ Mes Likes
    & Affiche toutes les photos likées par le compte connecté. Les IDs sont
      persistés en \ic{SharedPreferences} sous la clé \ic{liked\_ids\_\{username\}}
      (isolation par compte — changer de session ne transfère pas les likes).
      Le chip appelle \ic{viewModel.getPhotosByIds(ids)} qui exécute un
      \ic{SELECT ... WHERE id IN (:ids)} en Room. \\
  Flux aléatoire
...
\ic{ExplorerFragment} implémente l'interface \ic{SensorEventListener}, qui
définit deux méthodes de callback : \ic{onSensorChanged(SensorEvent event)}
appelée à chaque nouvelle mesure, et \ic{onAccuracyChanged(Sensor, int)} pour
les changements de précision.

Le capteur est obtenu via \ic{SensorManager} (service système récupéré avec
\ic{getSystemService(Context.SENSOR\_SERVICE)}) et instancié avec
\ic{getDefaultSensor(Sensor.TYPE\_ACCELEROMETER)}. L'enregistrement de l'écouteur
se fait dans \ic{onResume()} avec le taux \ic{SENSOR\_DELAY\_UI} (faible
consommation, adapté aux applications sans besoin de haute précision), et le
désenregistrement dans \ic{onPause()} pour libérer les ressources et économiser
la batterie — conformément aux bonnes pratiques de gestion du cycle de vie.

Le shake est détecté en calculant la variation d'accélération sur les trois axes
(\ic{event.values[0/1/2]}) entre deux mesures consécutives. Si la variation
dépasse \ic{SHAKE\_THRESHOLD} sur au moins deux axes simultanément, et que le
cooldown de 1500 ms est écoulé, le shake est validé.
...
\begin{itemize}
  ...
  \item \textbf{Modifier le titre} (si auteur) : si l'utilisateur connecté est
    ...
    qui met à jour Room \textbf{et} le champ \ic{title} dans Firestore.
  \item \textbf{Supprimer sa photo} (si auteur) : le bouton "🗑 Supprimer" ouvre
    ...
    appelé — supprime dans Room \textbf{et} dans Firestore (\ic{photos/\{photoId\}}).
  \item \textbf{Supprimer ses commentaires} : le \ic{CommentAdapter} reçoit
    ...
    \ic{viewModel.deleteComment(id)} — récupère d'abord le commentaire en Room puis
    le supprime dans Room \textbf{et} dans Firestore (requête par \ic{photoId+author+text}).
\end{itemize}
...
\begin{itemize}
  \item \textbf{null} (non membre) : bouton "Demander →" — appelle
    \ic{requestJoinGroup()} (Room) + \ic{FirebaseRepository.saveGroupMember(..., "PENDING")}.
    Une \ic{AppNotification} est envoyée au créateur.
  \item \textbf{PENDING} : bouton "⏳ En attente" non cliquable.
  \item \textbf{MEMBER} : bouton "Membre ✓" — clic propose de quitter via
    \ic{rejectOrLeaveGroup()} (Room) + \ic{deleteGroupMember()} (Firestore).
\end{itemize}

L'onglet "Mes groupes" appelle \ic{viewModel.syncMyMemberStatuses(username)} au chargement :
si une demande a été acceptée depuis un autre appareil (statut MEMBER dans Firestore),
Room est mis à jour et le groupe apparaît dans la liste.

Pour créer un groupe, un \ic{AlertDialog} demande nom + description, puis
appelle \ic{viewModel.insertGroup(group)} (Room) + \ic{FirebaseRepository.saveGroup()}
(Firestore), et bascule sur l'onglet "Mes groupes".
...
\begin{itemize}[topsep=2pt]
  \item \ic{listenToMessages(groupName, ...)} : reçoit en temps réel tous les messages
    des autres utilisateurs, les insère dans Room s'ils sont absents. LiveData Room
    met l'UI à jour automatiquement.
  \item \ic{listenToPendingMembers(groupName, ...)} (créateur uniquement) : reçoit les
    nouvelles demandes d'adhésion depuis d'autres appareils, les insère dans Room.
\end{itemize}
Les deux listeners sont retirés dans \ic{onDestroyView()} pour éviter les fuites mémoire.
...
via \ic{viewModel.insertAppNotification(notif)}. Les événements cross-appareils
(like reçu sur une photo, adhésion acceptée) sont également écrits dans Firestore
(\ic{saveNotification(targetUsername, notif)}). Un listener Firestore dans
\ic{NotificationsFragment} (\ic{listenToNotifications()}) reçoit les notifications
des autres appareils et les insère dans Room — l'utilisateur est notifié même
s'il est connecté sur un second appareil. Le listener est retiré dans \ic{onDestroyView()}.
Un badge rouge sur la cloche de \ic{MainActivity} compte les non-lues via
\ic{getUnreadNotificationCount(userId)}.
...
\ic{AUTEUR}, \ic{LIEU}, \ic{TAG} et \ic{GROUPE}.
Chaque ajout est écrit dans Firestore (\ic{notification\_prefs}), chaque suppression
aussi. À l'ouverture du fragment, \ic{syncNotificationPreferences()} récupère
les préférences sauvegardées sur d'autres appareils.
...
La biographie est un \ic{EditText} pré-rempli depuis Room au chargement du fragment.
Si Room est vide, \ic{FirebaseRepository.loadUserProfile(username)} est appelé et
remplit le champ depuis Firestore (bio sauvegardée depuis un autre appareil).
Un bouton "OK" déclenche la sauvegarde dans Room \textbf{et} dans Firestore
(\ic{saveUserProfile(username, bio)}), rendant la bio disponible sur tous les appareils.
...
la mise à jour en base via \ic{planDao.updatePlan()} et, si \ic{plan.liked = true},
sauvegarde le plan dans Firestore (\ic{plans/\{username\}\_\{id\}}) comme backup cloud.
Le plan apparaît alors dans l'onglet "Sauvegardés ❤️" de \ic{TravelPathFragment}.
...
La base de données \ic{tp3\_database} (Room v20) contient 12 tables. Chaque table
...
\begin{longtable}{>{\ttfamily\small}l >{\small}p{6.5cm} >{\centering\small}p{1.8cm}}
  ...
  \endhead
  User
    & id, login (unique), password, nom, prenom, email,
      telephone, dateNaissance, centresInteret,
      \textbf{avatarUri} (photo de profil), \textbf{bio} (biographie).
      Stocke les comptes utilisateurs. L'identité cloud est gérée
      par Firebase Auth (email fictif \ic{login@traveling.app}).
    & Commun \\
  Photo
  ...
  GroupMember
    ...
    & TS \\
  GroupMessage
    & id, groupId, userId, authorName, message, date,
      \textbf{photoId} (0 = texte, $>$0 = photo partagée).
      Messages du chat de groupe — 3 types selon photoId.
    & TS \\
  Report
...
@Database(
    ...
    version = 20,        // incremente a chaque changement de schema
    exportSchema = false
)
...
L'application \textbf{Traveling} répond aux objectifs fixés en début de projet : proposer
une plateforme de partage de photos de voyage et un générateur d'itinéraires personnalisés,
le tout intégré dans une architecture mobile cohérente et évolutive.

Sur le plan technique, le projet mobilise les principaux concepts du module — MVVM, Room,
capteurs, notifications, cartographie — et y ajoute une dimension réseau complète via
Firebase Firestore, permettant une synchronisation multi-appareils en temps réel.
L'ensemble repose exclusivement sur des technologies gratuites et open-source.

Des améliorations restent envisageables, notamment la synchronisation des images entre
appareils (nécessiterait Firebase Storage avec un plan payant) ou l'ajout de
notifications push via Firebase Cloud Messaging. Ces extensions n'ont pas été retenues
dans le cadre de ce projet afin de respecter la contrainte de gratuité totale.
...
de l'annotation IA, de la pagination, du partage, un profil utilisateur.
Une synchronisation Firebase complète sur 10 collections Firestore : photos, likes,
commentaires, groupes, membres, messages de chat (temps réel), notifications in-app,
préférences, profils et plans sauvegardés — le tout sans aucun service payant,
développé en Java pur en suivant le pattern MVVM.
\end{okbox}
...
\begin{tabularx}{\textwidth}{l X l}
  ...
  Firebase Auth      & Authentification email/password, gestion des sessions
                     & Gratuit (Spark) \\
  Firebase Firestore & Synchronisation cloud temps réel : photos, likes, commentaires,
                       groupes, membres, messages de chat, notifications, préférences,
                       profils, plans sauvegardés (10 collections)
                     & Gratuit (Spark) \\
