...
\begin{tabularx}{\textwidth}{>{\bfseries}l X}
  ...
  Firebase Auth + Firestore
    & Authentification email/password et synchronisation cloud des données sociales
      (photos, likes, commentaires) via \ic{addSnapshotListener} pour des mises à jour
      temps réel. Security Rules configurées (\ic{request.auth != null}).
      Plan Spark gratuit — aucune carte bancaire requise. \\
  FirebaseRepository
    & Pattern Repository (recommandé par le cours) : toutes les opérations Firestore
      ...
      complètement Firebase de l'UI. Les Activities n'accèdent plus directement
      au SDK Firestore. \\
  Room (SQLite local)
    & Architecture \textbf{local-first} : les données privées (groupes, chat, parcours,
      préférences, notifications) restent sur l'appareil de l'utilisateur.
      Ce choix délibéré sépare la \textit{couche sociale} (contenu public synchronisé
      via Firestore) de la \textit{couche personnelle} (données sensibles jamais
      transmises à un serveur tiers). Room garantit également le fonctionnement
      hors-ligne et offre sécurité de type et LiveData automatiques. \\
...
En vue de l'ampleur fonctionnelle du projet, nous avons priorisé la synchronisation
cloud des fonctionnalités sociales principales via Firebase. Certaines fonctionnalités
secondaires comme les groupes locaux ou les préférences ont été maintenues en stockage
local afin de limiter la complexité backend.

\begin{tabularx}{\textwidth}{>{\bfseries}l X}
  \toprule
  Donnée & Stratégie \\
  \midrule
  Authentification & Firebase Auth (email/password). Email fictif \ic{login@traveling.app}
                     pour conserver l'UX username. \\
  Photos publiées  & Firestore collection \ic{photos}, document ID = ID Room.
                     Champs : titre, lieu, auteur, coordonnées, catégorie, tags, likes. \\
  Likes            & Champ \ic{likes} mis à jour dans Firestore à chaque clic.
                     \ic{addSnapshotListener} sur le document photo : le compteur se
                     rafraîchit en temps réel sans action utilisateur. \\
  Commentaires     & Collection Firestore \ic{comments} avec champ \ic{photoId}.
                     \ic{addSnapshotListener} sur la collection filtrée : les nouveaux
                     commentaires apparaissent instantanément et sont synchronisés dans Room. \\
  Groupes / Chat   & Room local — données privées, fonctionnelles sur un appareil. \\
  TravelPath       & Room local — itinéraires personnels, pas de partage nécessaire. \\
  \bottomrule
\end{tabularx}

\textbf{Architecture} : toutes les opérations Firestore sont centralisées dans
\ic{FirebaseRepository} (pattern Repository, singleton), conformément aux bonnes pratiques
du cours. Les Activities et Fragments n'accèdent jamais directement au SDK Firestore.

\textbf{Security Rules} : les règles Firestore sont configurées avec
\ic{allow read, write: if request.auth != null} — seuls les utilisateurs authentifiés
peuvent lire et écrire. Le mode test (ouvert) n'est pas utilisé en production.
